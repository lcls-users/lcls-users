...
| 2. Fast Feedback | diagnostics and results<br> within <1min.| <1 week. | through `s3dflogin`, then on either interactive or compute node, do `export SIT_PSDM_DATA=/sdf/data/lcls/drpsrcf/ffb` |
| 3. Offline Analysis | intensive compute during<br> and after the beamtime.| <4 months<br> before being moved to tape.| through `s3dflogin`, then on either interactive or compute node, do `export SIT_PSDM_DATA=/sdf/data/lcls/ds`          |
